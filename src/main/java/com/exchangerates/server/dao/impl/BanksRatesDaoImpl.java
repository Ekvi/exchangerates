   ...
   /* @Override
    public void update(Map<String, Rates> values) {
        ...
        try {
            ...
            for(Map.Entry<String, Rates> map : values.entrySet()) {
                Rates rates = map.getValue();
                Rates fromDb = (Rates)session().createCriteria(entityClass)
                            .add(Restrictions.eq("name", rates.getName())).uniqueResult();
